...
test('worker.updateSettings() rejects with WorkerClosedError if closed', async () => {
    ...
    worker.close();
    await (0, enhancedEvents_1.enhancedOnce)(worker, 'subprocessclose');
    await expect(worker.updateSettings({ logLevel: 'error' })).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
...
test('worker.dump() rejects with WorkerClosedError if closed', async () => {
    ...
    worker.close();
    await (0, enhancedEvents_1.enhancedOnce)(worker, 'subprocessclose');
    await expect(worker.dump()).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
