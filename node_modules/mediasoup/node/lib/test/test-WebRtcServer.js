...
test('worker.createWebRtcServer() rejects with WorkerClosedError if Worker is closed', async () => {
    ...
    })).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
test('webRtcServer.close() succeeds', async () => {
    ...
    webRtcServer.close();
    await expect(webRtcServer.dump()).rejects.toThrow(errors_1.NotFoundError);
...
test('WebRtcServer emits "workerclose" if Worker is closed', async () => {
    ...
    expect(onObserverClose).toHaveBeenCalledTimes(1);
    expect(webRtcServer.closed).toBe(true);
    await expect(webRtcServer.dump()).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
