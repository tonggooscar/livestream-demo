...
test('WebRtcTransport methods reject if closed', async () => {
    ...
    await expect(webRtcTransport.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(webRtcTransport.getStats()).rejects.toThrow(errors_1.NotFoundError);
    // @ts-expect-error --- Testing purposes.
    await expect(webRtcTransport.connect({})).rejects.toThrow(TypeError);
    await expect(webRtcTransport.setMaxIncomingBitrate(200000)).rejects.toThrow(errors_1.NotFoundError);
    await expect(webRtcTransport.setMaxOutgoingBitrate(200000)).rejects.toThrow(errors_1.NotFoundError);
    await expect(webRtcTransport.setMinOutgoingBitrate(100000)).rejects.toThrow(errors_1.NotFoundError);
    await expect(webRtcTransport.restartIce()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
...
test('WebRtcTransport emits "routerclose" if Worker is closed', async () => {
    ...
    expect(webRtcTransport.dtlsState).toBe('closed');
    expect(webRtcTransport.sctpState).toBeUndefined();
    await expect(webRtcTransport.dump()).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
