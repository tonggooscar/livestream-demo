...
test('dataProducer.close() succeeds', async () => {
    ...
    await expect(dataProducer1.dump()).rejects.toThrow(errors_1.NotFoundError);
...
test('DataProducer methods reject if closed', async () => {
    ...
    dataProducer1.close();
    await expect(dataProducer1.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(dataProducer1.getStats()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
test('DataProducer emits "transportclose" if Transport is closed', async () => {
    ...
    await expect(dataProducer2.dump()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
