...
test('PlainTransport methods reject if closed', async () => {
    ...
    await expect(plainTransport.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(plainTransport.getStats()).rejects.toThrow(errors_1.NotFoundError);
    await expect(plainTransport.connect({})).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
...
test('PlainTransport emits "routerclose" if Worker is closed', async () => {
    ...
    expect(onObserverClose).toHaveBeenCalledTimes(1);
    expect(plainTransport.closed).toBe(true);
    await expect(plainTransport.dump()).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
