...
test('DirectTransport methods reject if closed', async () => {
    ...
    expect(directTransport.closed).toBe(true);
    await expect(directTransport.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(directTransport.getStats()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
...
test('DirectTransport emits "routerclose" if Worker is closed', async () => {
    ...
    expect(onObserverClose).toHaveBeenCalledTimes(1);
    expect(directTransport.closed).toBe(true);
    await expect(directTransport.getStats()).rejects.toThrow(errors_1.WorkerClosedError);
}, 2000);
