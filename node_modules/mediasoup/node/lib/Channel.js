...
class Channel extends enhancedEvents_1.EnhancedEventEmitter {
    ...
    notify(event, bodyType, bodyOffset, handlerId) {
        ...
        if (this.#closed) {
            throw new errors_1.WorkerClosedError(`Channel closed, cannot send notification [event:${notification_1.Event[event]}]`);
        }
    ...
    async request(method, bodyType, bodyOffset, handlerId) {
        ...
        if (this.#closed) {
            throw new errors_1.WorkerClosedError(`Channel closed, cannot send request [method:${request_1.Method[method]}]`);
        }
        ...
        return new Promise((pResolve, pReject) => {
            const sent = {
                ...
                close: () => {
                    pReject(new errors_1.WorkerClosedError(`Channel closed, pending request aborted [method:${request_1.Method[method]}, id:${id}]`));
                },
            };
    ...
    processResponse(response) {
        ...
        if (response.accepted()) {
            ...
            switch (response.error()) {
                ...
                case 'NotFoundError': {
                    sent.reject(new errors_1.NotFoundError(response.reason()));
                    break;
                }
                default: {
